...
- **Spot-the-lie** — the boss's mechanic: each boss steady slot gives four short statements on the student's weak topic, exactly one FALSE; pick the lie. Tests whether a plausible-but-wrong idea (the exam trap) slips past — but delivered simply (four one-line statements), so a *struggling* student can read it fast and gain confidence when they catch it. Attacks span the student's DIFFERENT weak subjects, not one. Reuses plain MC schema (statements as options), so no shell/validator cost; composition doctrine in planner (boss branch), review has spot-the-lie category mapping (two-lies, or keyed-statement-actually-true, must be caught). Replaced spot-the-flaw, which was too heavy to read for the kid it targets.
