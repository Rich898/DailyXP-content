...
- `template_v3.html` — canonical shell. ⚠ A template fix reaches NOBODY until all three sites are re-deployed — the shells are static copies, not readers of this repo (the 31 Aug Scrub-It lesson). **Primary deploy path:** run the `deploy-shells` workflow (Actions → deploy-shells → Run workflow, or let a session dispatch it) — it re-reads each kid's name from the live page, stamps, API-deploys all three sites and verifies the new version is live (`tools/deploy_shells.py`; uses `NETLIFY_AUTH_TOKEN`). **Manual fallback:** `python3 tools/stamp_shell.py --names "y8=<Name>,y9=<Name>,t1=<Name>"` writes one drag-deploy zip per seat into gitignored `shell/build/`; drag each onto its Netlify project (Deploys → drag zip → same project, URL unchanged).
